...
class CrashEngine:
    ...
    def generate_crash_point(server_seed, client_nonce=None):
        ...
        Provably fair: uses HMAC-SHA256 of server seed + client nonce
        """
        ...
        # Extract entropy from hash
        byte1 = int(hash_result[0:8], 16)
        random_factor = (byte1 % 10000) / 10000.0
        
        # Exponential distribution to favor lower multipliers (realistic crash odds)
        crash_multiplier = math.exp(random_factor * math.log(float(CrashEngine.MAX_MULTIPLIER)))
        crash_multiplier = max(float(CrashEngine.MIN_MULTIPLIER), 
                               min(float(CrashEngine.MAX_MULTIPLIER), crash_multiplier))
        ...
        try:
            with app.app_context():
                ...
                while time.time() - start_time < game_duration:
                    ...
                    # Linear progression to crash point
